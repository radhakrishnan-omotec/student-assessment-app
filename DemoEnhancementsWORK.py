...
# Constants
#CSV_FILE_PATH = 'Final Culmination_AI_Blocks_1.csv'
...
def generate_blooms_taxonomy_graphs():
    ...
    fig, axs = plt.subplots(1, len(blooms_columns), figsize=(25, 5))
    for i, (column, label) in enumerate(blooms_columns):
        if column in df.columns:
            st.markdown(f"<h2 style='text-align:left; font-size:28px;'>{label}</h2>", unsafe_allow_html=True)
...
def generate_grade_comparison_graphs():
    ...
    for metric in numeric_cols:
        st.markdown(f"<h2 style='font-size:28px;'>Metric: {metric}</h2>", unsafe_allow_html=True)
        fig, ax = plt.subplots(figsize=(10, 4))
        sns.barplot(data=df, x=grade_col, y=metric, ax=ax)
